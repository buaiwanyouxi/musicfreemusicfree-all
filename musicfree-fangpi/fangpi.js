// 放屁音乐网 (fangpi) MusicFree 插件
// 后端：放屁音乐网 https://www.fangpi.net
//   （与「歌曲宝」gequbao.com 同引擎、同接口，仅 BASE/PLATFORM 不同；本文件为放屁音乐网版）
//
// 取链原理（已逆向验证）：
...
(function () {
  ...
  if (!reqFn) {
    throw new Error('[fangpi] 插件沙箱未提供 require，无法加载');
  }
  var axios = reqFn('axios');

  var PLATFORM = 'fangpi';
  var BASE = 'https://www.fangpi.net';
  var UA =
  ...
  function getTopLists() {
    ...
    return Promise.resolve([{ title: '放屁音乐网 · 榜单', data: data }]);
  }
  ...
  var plugin = {
    ...
    srcUrl: 'https://cdn.jsdelivr.net/gh/buaiwanyouxi/musicfreemusicfree-all@main/musicfree-fangpi/fangpi.js',
