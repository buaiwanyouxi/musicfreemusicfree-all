# 放屁音乐网 MusicFree 插件 (fangpi.js)

将 [放屁音乐网](https://www.fangpi.net) 适配为 MusicFree 插件。数据源为酷我音乐（KuWo）CDN 直链，无需登录。

> 放屁音乐网与「歌曲宝」(gequbao.com) 为**同引擎、同接口**（均使用 `gequbao.com` 系接口），取链流程完全一致；本插件与 `gequbao.js` 代码逻辑相同，仅数据源站点不同。
...
放屁音乐网不提供直链接口，取链为「歌曲页拿会话 → 解密令牌 → 接口换链」三步：
...
1. 复制以下 raw 链接：
   ```
   https://cdn.jsdelivr.net/gh/buaiwanyouxi/musicfreemusicfree-all@main/musicfree-fangpi/fangpi.js
   ```
...
### 方式二：本地加载（开发调试）
- 直接用支持的本地插件加载方式指向本目录的 `fangpi.js`。
